...
public class HeapSettleStrategy implements SettleStrategies {

    @Override
    public List<Transaction> settleUpGroup(Map<String, Integer> map) {

        PriorityQueue<Pair> receivers = new PriorityQueue<>();
        PriorityQueue<Pair> givers = new PriorityQueue<>(Collections.reverseOrder());
        ...
        for (String user : map.keySet()) {
            ...
            if (amount < 0) {
                givers.add(new Pair(amount, user));
            } else if (amount > 0) {
